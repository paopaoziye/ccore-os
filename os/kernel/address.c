...
PhysPageNum phys_page_num_from_size_t(uint64_t v){
    PhysPageNum pageNum;
    //获取低 PPN_WIDTH_SV39 位为1的掩码从而提取物理页号
...
VirtAddr virt_addr_from_size_t(uint64_t v){
    VirtAddr addr;
    //获取低 VA_WIDTH_SV39 位为1的掩码从而提取虚拟地址
...
VirtPageNum virt_page_num_from_size_t(uint64_t v){
    VirtPageNum pageNum;
    //获取低 VPN_WIDTH_SV39 位为1的掩码从而提取虚拟页号
...
VirtPageNum ceil_virt(VirtAddr virt_addr){
    VirtPageNum virt_page_num;
    virt_page_num.value = (virt_addr.value + PAGE_SIZE - 1) / PAGE_SIZE;
    return virt_page_num;
}
